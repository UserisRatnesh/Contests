  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      if (waterStartTime[j] <= endTime) {
        int currAns = endTime + waterDuration[j];
        ans = min(ans, currAns);
      } else {
        int currAns = waterStartTime[j] + waterDuration[j];
        ans = min(ans, currAns);
      }
    }
  }
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < m; j++) {
      ...
      if (landStartTime[j] <= endTime) {
        int currAns = endTime + landDuration[j];
        ans = min(ans, currAns);
      } else {
        int currAns = landStartTime[j] + landDuration[j];
        ans = min(ans, currAns);
      }
    }
  }
